...
clientkeys = gears.table.join(
  ...
  end, { description = "switch slave and master", group = "client" }),
...
function myfocus_filter(c)
  if old_filter(c) then
    ...
    if
      (c.instance == "tim.exe" or c.instance == "TIM.exe")
      and c.above
      and c.skip_taskbar
      and (c.type == "normal" or c.type == "dialog") -- dialog type is for tooltip windows
      and (c.class == "qq.exe" or c.class == "QQ.exe" or c.class == "TIM.exe")
    then
      return nil
      -- This works with tooltips and some popup-menus
    elseif c.class == "Wine" and c.above == true then
      return nil
    elseif c.class == "WeChat.exe" and (c.name == "CMenuWnd" or not c.name or c.name == "SessionChatRoomDetailWnd") then -- 微信菜单
      c.border_width = 0
      do
        return nil
      end
    elseif
      (c.class == "Wine" or c.class == "QQ.exe" or c.class == "qq.exe")
      and c.type == "dialog"
...
awful.rules.rules = {
  ...
  { rule = { class = "scrcpy" }, properties = { screen = 1, tag = "7 😃 " } },
  {
